...
- **2026-05-14 (v1.04 Release):** New Covenant v1.04 release. Updated service worker CACHE_NAME to v1.04. Updated Wellspring IndexedDB version to 104. Updated Unity footer version strings across all apps to v1.04. Fixed FlockChat VAPID key to use centralized `window.FLOCK_VAPID_KEY` configuration (prevents FCM token generation errors). Complete site audit performed: all 8 app shells, 9 embed files, root launcher, and core scripts verified error-free. Created comprehensive Q-AS-BUILT-App-Dependencies.md documenting complete script loading order and dependency chains. Release includes commits b335bb95 (Q-AS-BUILT creation), bafa8fe2 (FlockChat VAPID fix), and this version bump.

...
| `app.flockchat/` | `app.flockchat.html` / `index.html` | FlockChat — Church team messaging PWA. Auth-gated standalone shell. `flockchat.js` provides channels, DMs, and real-time typing via direct Firestore compat SDK (no RTDB required; heartbeat-based typing TTL). Firebase config defaults to `null` (uses shared flockos-comms project); standalone HTML is **not** patched by B-Build. (B-Build step 9e patches the embed wrapper `embed-flockchat.html`, not this file.) | Y | `manifest.json` |

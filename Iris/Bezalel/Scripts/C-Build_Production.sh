#!/usr/bin/env bash
# ======================================================================
# C-Build_Production.sh — Build New_Covenant for production Nations
#
# Source of truth:  New_Covenant/
# Output:           Nations/<church>/   (repo root)
#
# Production Nations: Root, FlockOS, GAS
# Excluded: TBC, TheForest
#
...
#
# Usage:
#   bash Iris/Bezalel/Scripts/C-Build_Production.sh
#   bash Iris/Bezalel/Scripts/C-Build_Production.sh --dry-run
# ======================================================================
set -euo pipefail
...
# PRODUCTION NATIONS ONLY — excludes TBC and TheForest
CHURCHES=(
...
echo "══════════════════════════════════════════════"
echo "Production Nations build complete:"
for entry in "${CHURCHES[@]}"; do
  IFS='|' read -r FOLDER CONFIG CACHE <<< "$entry"
  echo "  Nations/$FOLDER/"
done
...
if ! $DRY_RUN; then
  ...
  if [ -f "$SEED_SCRIPT" ]; then
    ...
    GIT_SUBJECT=$(git -C "$WORKSPACE" log -1 --pretty=format:'%s' 2>/dev/null || echo "C-Build")
    GIT_HASH=$(git -C "$WORKSPACE" log -1 --pretty=format:'%h' 2>/dev/null || echo "")
    BUILD_DATE=$(date +%Y-%m-%d)
    BUILD_TITLE="Build: ${GIT_SUBJECT}"
    BUILD_DESC="C-Build (Production) completed on ${BUILD_DATE}. Nations synced: Root, FlockOS, GAS. Commit: ${GIT_HASH}. ${GIT_SUBJECT}"
